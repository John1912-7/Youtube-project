...
## GitHub и публикация сайта

Текущий репозиторий: `John1912-7/Youtube-project`.

Сейчас в репозитории используются две ветки:

- `main` - рабочая ветка проекта. Здесь лежат README, docs, `.env.example`, текущий статический прототип сайта и будущий код приложения.
- `gh-pages` - ветка только для GitHub Pages. Она нужна, чтобы публиковать сайт в интернет через GitHub Pages.

Почему появилась ветка `gh-pages`:

- GitHub Pages умеет публиковать сайт из отдельной ветки.
- Для публичной страницы не нужно публиковать всю структуру проекта как сайт.
- В `gh-pages` был положен минимальный static build: `index.html` и `.nojekyll`.
- Это делает GitHub Pages проще: он берет `index.html` из корня `gh-pages` и показывает его как сайт.

Важный момент про приватность:

- Ветки не бывают private/public отдельно.
- Видимость задается всему repository.
- Если repository public, люди могут видеть и `main`, и `gh-pages`.
- Если repository private, люди не видят код, но GitHub Pages на GitHub Free не работает как публичный сайт для private repo.

Почему мы временно сделали этот repo public:

- Нужно было быстро опубликовать первый статический сайт бесплатно.
- Пользователь не хотел создавать отдельный public repo только для сайта.
- Поэтому был выбран компромисс: один repo, `main` как рабочая ветка, `gh-pages` как ветка публикации.

Лучший вариант позже:

- Вернуть основной продуктовый repo в private.
- Создать отдельный public repo только для landing/static site.
- В public repo держать только безопасные файлы сайта.
- В private repo держать backend, docs, business plan, сервисную стратегию и весь реальный продуктовый код.

Ожидаемый GitHub Pages URL:

```text
https://john1912-7.github.io/Youtube-project/
```

Если URL дает `404`, проверить настройки:

1. Открыть `Settings -> Pages`.
2. Выбрать `Deploy from a branch`.
3. Branch: `gh-pages`.
4. Folder: `/ (root)`.
5. Нажать `Save` и подождать 1-3 минуты.
...
- GitHub repo создан и временно открыт как public для GitHub Pages;
- `main` содержит рабочую документацию и текущий прототип;
- `gh-pages` содержит минимальную публичную версию сайта для GitHub Pages;
- статический сайт-прототип создан: `index.html`, `styles.css`, `app.js`;
- следующий шаг - включить GitHub Pages в настройках или перейти к scaffold Next.js app.
